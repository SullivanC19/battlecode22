...
public class Sage {
    public static void run() throws GameActionException {
        ...
        int maxHealth = RobotType.SAGE.health;
        ...
        RobotInfo[] enemyRobots = Memory.rc.senseNearbyRobots(
                RobotType.SAGE.visionRadiusSquared, Memory.rc.getTeam().opponent());
        ...
        if (enemyRobots.length > 0 && Memory.rc.getActionCooldownTurns() < GameConstants.COOLDOWN_LIMIT) {
            micro(enemyRobots);
        } else {
            ...
            if (closestEnemyBlock != null && closestEnemyBlock.equals(myBlock)) {
                ...
                if (Memory.rc.getActionCooldownTurns() >= GameConstants.COOLDOWN_LIMIT) {
                    closestEnemyBlock = closestEnemyBlock.add(closestEnemyBlock.directionTo(archonBlock));
                }
    ...
    private static void micro(RobotInfo[] enemyRobots) throws GameActionException {
        ...
        RobotInfo[] enemyRobotsInActionRadius = Memory.rc.senseNearbyRobots(
                RobotType.SAGE.actionRadiusSquared, Memory.rc.getTeam().opponent());
        ...
        for (Direction dir : Direction.allDirections()) {
            ...
            RobotInfo[] eriar = Memory.rc.senseNearbyRobots(
                    adjLoc, RobotType.SAGE.actionRadiusSquared, Memory.rc.getTeam().opponent());
            for (RobotInfo ri : eriar) {
                if (ri.getType() == RobotType.MINER || ri.getType() == RobotType.BUILDER) continue;
