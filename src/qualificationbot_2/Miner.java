    ...
    public static void run() throws GameActionException {
        ...
        for (int i = 0; i < leadLocs.length; i++) {
            ...
            if (lead >= MIN_TARGETABLE_LEAD
                ...
                adjacentMineLoc |= leadLocs[i].isAdjacentTo(myLoc);
                if (closestEnemyThreat == null || closestEnemyThreat.getLocation().distanceSquaredTo(leadLocs[i]) > closestEnemyThreat.getType().actionRadiusSquared) {
        ...
        if (closestEnemyThreat != null && !adjacentMineLoc) {
            ...
            if (closestFriendlySoldierLoc != null) {
                Pathfinder.moveTo(closestFriendlySoldierLoc.add(closestFriendlySoldierLoc.directionTo(Memory.archonLocation)));
            } else {
            ...
            for (int i = 0; i < Communication.NUM_TARGET_BLOCKS; i++) {
                ...
                MapLocation target = Utils.getCenterOfBlock(targetMiningBlock);
                int dist = Utils.dist(target, myLoc) + Utils.dist(Memory.archonLocation, target) / 3;
            ...
            MapLocation center = new MapLocation(Memory.rc.getMapWidth() / 2, Memory.rc.getMapHeight() / 2);

            // explore blocks
            for (int i = 0; i < Communication.NUM_TARGET_BLOCKS; i++) {
                MapLocation targetExploreBlock = Communication.getTargetExploreBlock(i);
                if (targetExploreBlock == null) continue;
                MapLocation target = Utils.getCenterOfBlock(targetExploreBlock);
                int dist = Utils.dist(target, myLoc) - Utils.dist(Memory.archonLocation, target) / 3;
                if (dist < distToTargetBlock) {
