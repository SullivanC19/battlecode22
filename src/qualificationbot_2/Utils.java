...
public class Utils {
    ...
    public static final int MAX_NUM_BLOCKS = (MAX_MAP_SIZE / BLOCK_SIZE) * (MAX_MAP_SIZE / BLOCK_SIZE);
    ...
    public static boolean tryBuild(RobotType robotType) throws GameActionException {
        return tryBuild(robotType, null);
    }

    public static boolean tryBuild(RobotType robotType, Direction startDir) throws GameActionException {
        if (startDir == null || startDir == Direction.CENTER) {
            startDir = Utils.randomDirection();
        }

        for (Direction dir = startDir; dir.rotateLeft() != startDir; dir = dir.rotateLeft()) {
    ...
    public static int dist(MapLocation l1, MapLocation l2) {
