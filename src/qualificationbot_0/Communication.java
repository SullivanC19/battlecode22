...
public class Communication {
    ...
    public static final int TARGET_MINING_BLOCKS_PICKED_SET_SIZE = 2 * Utils.MAX_NUM_ARCHONS; // relies on being divisible by 16 for easy zeroing
    public static final int TARGET_BLOCKS_SIZE = TARGET_MINING_BLOCKS_SIZE + TARGET_ENEMY_BLOCKS_SIZE + TARGET_MINING_BLOCKS_PICKED_SET_SIZE;
    ...
    public static final int TARGET_MINING_PICKED_SET_START_BIT = TARGET_ENEMY_BLOCKS_START_BIT + TARGET_ENEMY_BLOCKS_SIZE;

    // enemy archon locations (32)
    ...
    public static final int NO_LEAD_BLOCKS_SIZE = BLOCK_INFO_ARRAY_COUNT_SIZE + BLOCK_LOC_SIZE * 15;
    public static final int NO_ENEMY_BLOCKS_SIZE = BLOCK_INFO_ARRAY_COUNT_SIZE + BLOCK_LOC_SIZE * 15;
    public static final int MINEABLE_BLOCKS_SIZE = BLOCK_INFO_ARRAY_COUNT_SIZE + BLOCK_LOC_SIZE * 15;
    public static final int UNMINEABLE_BLOCKS_SIZE = BLOCK_INFO_ARRAY_COUNT_SIZE + BLOCK_LOC_SIZE * 15;
    public static final int ENEMY_THREAT_BLOCKS_SIZE = BLOCK_INFO_ARRAY_COUNT_SIZE + BLOCK_LOC_SIZE * 15;
    public static final int ENEMY_NONTHREAT_BLOCKS_SIZE = BLOCK_INFO_ARRAY_COUNT_SIZE + BLOCK_LOC_SIZE * 15;
    ...
    private static final int ROUNDS_BEFORE_REBROADCAST_BLOCK_TYPE = 100;
    ...
    public static void setTargetBlocks(MapLocation[] targetBlocks, boolean isMining) throws GameActionException {
        ...
        if (isMining) resetTargetMiningBlocksPicked(getArchonIdx());
    }

    private static void resetTargetMiningBlocksPicked(int archonIdx) throws GameActionException {
        writeValue(0, TARGET_MINING_PICKED_SET_START_BIT + archonIdx * 2, 2);
    }

    public static void setTargetMiningBlockPicked(int archonIdx, int idx) throws GameActionException {
        writeValue(1, TARGET_MINING_PICKED_SET_START_BIT + archonIdx * 2 + idx, 1);
    }
    ...
    public static void addBlockInfo(int blockType, MapLocation block) throws GameActionException {
        if (lastRoundBroadcasted[blockType][block.x][block.y] != 0 && lastRoundBroadcasted[blockType][block.x][block.y] < Memory.rc.getRoundNum() + ROUNDS_BEFORE_REBROADCAST_BLOCK_TYPE) return;
    ...
    public static int getArchonIdx() throws GameActionException {
        ...
        for (int i = 0; i < Memory.rc.getArchonCount(); i++) {
            if (Communication.getArchonBlock(i).equals(archonBlock)) {
                return i;
            }
        }
    ...
    public static boolean getTargetMiningBlockPicked(int archonIdx, int idx) throws GameActionException {
        return readValue(TARGET_MINING_PICKED_SET_START_BIT + archonIdx * 2 + idx, 1) == 1;
    }
