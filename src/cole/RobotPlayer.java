...
public strictfp class RobotPlayer {
    ...
    static float spawnx=-1.0f;
    static float spawny=-1.0f;
    static int made=0;
...
//            System.out.println("Age: " + turnCount + "; Location: " + rc.getLocation());
    ...
    float m=r.type==RobotType.ARCHON?1.0f:0.0f;
        ...
        for(int i=0;i<directions.length;i++){
            ...
            scoree+=-(((float) dir.dx)*xc+((float) dir.dy)*yc)*2.0f/((float) Math.sqrt(xc*xc+yc*yc))/((float) Math.sqrt(xc*xc+yc*yc))*armc;
            }
            if(farmc>0.0f){
...
//            System.out.println("I moved!");
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        float allyCount=0.0f;
        float rangeCount=2.0f;
        for(int i=0;i<rr.length;i++){
            RobotInfo r=rr[i];
            float dis=(float)Math.sqrt((float)(r.location.x-me.x)*(float)(r.location.x-me.x)+(float)(r.location.y-me.y)*(float)(r.location.y-me.y));
            if(r.team!=rc.getTeam()){
            ...
            if(r.type==RobotType.SOLDIER && dis>0.0f){
             enemyCount+=1.0f/dis;
            }
            }else{
                float m=r.type==RobotType.ARCHON?1.0f:0.0f;
        ...
        for(int i=0;i<directions.length;i++){
            ...
            float scoree=0.0f;
            scoree=rng.nextFloat()*0.0001f;
            float dd=(float) Math.sqrt(dir.dx*dir.dx+dir.dy*dir.dy);
            // float a=0.0f;//3.14159265f/3.0f;
            ...
            //     float oxc=me.x-spawnx;
            //     float oyc=me.y-spawny;
            //     float l=(float) Math.sqrt(oxc*oxc+oyc*oyc);
            //     oxc/=l;
            //     oyc/=l;
            //     float f=((float)Math.signum(allyCount-enemyCount))*1.0f;
            //     float xc=oxc*f;
            //     float yc=oyc*f;
                
            // scoree+=((float) dir.dx)*xc+((float) dir.dy)*yc;
            // }
            if( pcnc>0.0f && allyCount-Math.max(rangeCount,enemyCount)>0.0f){
                //*(pcnc+pnc)/2.0f

                float oxcg=me.x-spawnx;
                float oycg=me.y-spawny;
                float gl=(float) Math.sqrt(oxcg*oxcg+oycg*oycg);
                
                oxcg/=gl;
                oycg/=gl;
                float mm=(float) Math.sqrt(pcnx/pcnc*pcnx/pcnc+pcny/pcnc*pcny/pcnc);
                float d=(float)Math.signum(pcnx/pcnc*oycg-pcny/pcnc*oxcg);
                float oxc=oycg*d;
                float oyc=-oxcg*d;
                float l=(float) Math.sqrt(oxc*oxc+oyc*oyc);
                oxc/=l;
                oyc/=l;
                mm=8.0f;//mm<1.0f?mm:1.0f;
                float xc=-oxc*mm;
                float yc=-oyc*mm;
                
            scoree+=(((float) dir.dx)*xc+((float) dir.dy)*yc)/dd;
            }
            if(true){
                float oxc=me.x-spawnx;
                float oyc=me.y-spawny;
                ...
                oxc/=l;
                oyc/=l;
                float f=((float)Math.signum(allyCount-Math.max(rangeCount,enemyCount)))+(allyCount-Math.max(rangeCount,enemyCount));
                if(l<3.0f){
                    f=4.0f;
                }
                float xc=oxc*f;
